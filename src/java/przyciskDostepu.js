...
document.addEventListener('click', (e) => { // to musialo byc zmienione bo tak samo jak z ladowaniem gier, strona sie ladowala wolniej od skryptu
    const btn = e.target.closest('.accessibility'); // tutaj jest target closest bo jakby sie kliknelo w ten przycisk to to by nie dzialalo bo nie klikasz w przycisk tak jakby tylko w literke K wiec dzieki temu to idzie w gore tego drzewa i sprawdza czy jest tak klasa accessibility
    if (!btn) return;
