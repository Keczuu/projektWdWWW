...
async function seeGames() {
    ...
    if (!content) {
        console.error("Brakuje kontenera .games-content - nie mozna zaladowac gier");
        return;
    }
...
function render(content = document.querySelector('.games-content')) {
    if (!content) {
        return;
    }
...
function przyciskPoWiecej(content = document.querySelector('.games-content')) {
    let przycisk = document.getElementById('przyciskWiecej');
    if (zaladowane < wszystkieGry.length) {
        if (!przycisk) {
            const layout = content ? content.closest('.layout') : null;
            if (!layout) {
                return;
            }
